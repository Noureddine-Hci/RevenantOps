...
void UCharacterSelectWidget::NativeConstruct()
{
    ...
    if (BtnBack)
    {
        BtnBack->OnClicked.AddDynamic(this, &UCharacterSelectWidget::HandleBack);
    }
...
void UCharacterSelectWidget::BuildDefaultUI()
{
    ...
    Bg->SetBrushColor(FLinearColor(0.02f, 0.04f, 0.06f, 0.95f));
    ...
    Title->SetText(FText::FromString("SELECTION DU PERSONNAGE"));
    FSlateFontInfo TitleFont = Title->GetFont();
    TitleFont.Size = 28;
    Title->SetFont(TitleFont);
    Title->SetColorAndOpacity(FSlateColor(FLinearColor(0.6f, 1.f, 0.8f, 1.f)));
...
void UCharacterSelectWidget::PopulateCharacters(const TArray<FCharacterInfo>& Characters)
{
    ...
    for (int32 i = 0; i < Characters.Num(); ++i)
    {
        ...
        Card->Setup(i, Characters[i].DisplayName, Characters[i].Thumbnail);
        Card->OnCardClicked.AddUObject(this, &UCharacterSelectWidget::HandleCardClicked);
...
void UCharacterSelectWidget::HandleCardClicked(int32 Index)
{
    if (CachedCharacters.IsValidIndex(Index))
    {
        ...
        OnCharacterChosen.Broadcast(CachedCharacters[Index]);
    }
}
